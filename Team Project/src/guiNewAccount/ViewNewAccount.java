...
public class ViewNewAccount {
	...
	*/
	...
	private ViewNewAccount() {
    	...
    	label_NewUserCreation.getStyleClass().addAll("text-bold");
		...
		setupTextUI(text_Username, "Arial", 18, 300, Pos.BASELINE_LEFT, 275, 250, true);
		text_Username.setPromptText("Enter the Username");
		
		// Establish the text input operand field for the password
		setupTextUI(text_Password1, "Arial", 18, 300, Pos.BASELINE_LEFT, 275, 300, true);
		text_Password1.setPromptText("Enter the Password");
		
		// Establish the text input operand field to confirm the password
		setupTextUI(text_Password2, "Arial", 18, 300, Pos.BASELINE_LEFT, 275, 350, true);
        ...
        setupButtonUI(button_UserSetup, "Dialog", 18, 250, Pos.CENTER, 300, 470);
        button_UserSetup.getStyleClass().add("success");
        button_UserSetup.setOnAction((event) -> {ControllerNewAccount.doCreateUser(); });
		
        // Enable the user to quit the application
        setupButtonUI(button_Quit, "Dialog", 18, 250, Pos.CENTER, 300, 520);
        button_Quit.getStyleClass().add("danger");
