...
public class ViewUserLogin {
	...
	private static Label label_OperationalStartTitle = new Label("Log In or Use Invite Code");
	...
	private static Button button_ResetWithCode = new Button("Reset with Code");	// <-- NEW
	...
	private ViewUserLogin() {
		...
		theUserLoginScene.getStylesheets().add("file:src/cupertino-dark.css");
		...
		setupButtonUI(button_Login, "Dialog", 18, 200, Pos.CENTER, 475, 165);
		button_Login.getStyleClass().addAll("success");
		button_Login.setOnAction((event) -> {ControllerUserLogin.doLogin(theStage); });

		// NEW: Reset-with-code button (opens the OTP reset dialog)
		setupButtonUI(button_ResetWithCode, "Dialog", 18, 200, Pos.CENTER, 475, 205);
		button_ResetWithCode.getStyleClass().addAll("accent", "button-outlined", "flat");
		button_ResetWithCode.setOnAction(e -> ControllerUserLogin.openOtpReset(theStage));
		...
		button_SetupAccount.getStyleClass().addAll("accent");
		...
		theRootPane.getChildren().addAll(
				...
				button_Login, button_ResetWithCode,  // <-- NEW HERE
				text_Password, text_Invitation, button_SetupAccount,
				button_Quit);
	}
